...
NAV_MODE = "NAV_MODE" #NAV_MODE = "ALT_HOLD" HARDWARE "STABILIZE"
RTL_MODE = "RTL"
MANUAL_ANGLE = 4500
NAV_ANGLE = 2500 #Preliminary value for NAV_ANGLE.
desired_rate = -10 #cm/s
desired_height = 1000 #Stage 1
...
#pwm_throttle = PWM(inner_P_gain = 1, inner_I_gain = 0.5, outer_P_gain = 1.0, outer_D_gain = 1.0, vertical = True)
...
def descend_check(error_x, error_y, stage):
  ...
  '''
  if stage == 1:
    boundary_x, boundary_y = 30, 30
  elif stage == 2:
    boundary_x, boundary_y = 20, 20
  elif stage == 3:
    boundary_x, boundary_y = 10, 10
...
if __name__ == "__main__":
  ...
  try:
    ...
    while vehicle.armed:
      ...
      if searching: #HARDWARE #if not takeover:
        ...
        if not searching:
          ...
          if descend_check(error_x,error_y, stage):
            if do_descend_counter == 67: #(5sek/0.075 ≈ 67 run throughs)
              pwm_throttle.update(error_height)
              vehicle.channels.overrides[THROTTLE] = pwm_throttle.position 
              if h > 1000:
                stage = 1
                pwm_throttle.virtual_speed = 30
              elif 1000 > h > 300:
                stage = 2
                pwm_throttle.virtual_speed = 20
              elif h < 300:
                stage = 3
                pwm_throttle.virtual_speed = 15
              if stage == 3:
